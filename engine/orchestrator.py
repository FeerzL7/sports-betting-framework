"""
engine/orchestrator.py - Pipeline central del sistema
...
SOBRE el matching de games ↔ odds:
El Orchestrator necesita saber si el provider es un test double para
decidir la estrategia de matching. En vez de inspeccionar el nombre de
la clase (acoplamiento frágil), usa la propiedad `is_test_double`
definida en OddsProviderBase — contrato estable e inyectable.
...
class Orchestrator:
    ...
    def __init__(
        ...
        logger.info(
            ...
            extra={
                ...
                "odds_provider": type(odds_provider).__name__,
                "is_test_double": odds_provider.is_test_double,
            }
        )
    ...
    def run(
        ...
        logger.info(f"  Total exposure: {result.risk_status.get('total_exposure_pct', 0):.1f}%")
        logger.info("=" * 70)
    ...
    def _step_match_games(
        ...
        1) is_test_double=True (FakeOddsProvider) → matching posicional.
           Los scenarios están diseñados para coincidir 1:1 con los
           games que retorna el adapter. Pareamos por índice.

        2) is_test_double=False (producción) → matching por nombre normalizado.
        ...
        NOTA: Usamos self.odds_provider.is_test_double (contrato del
        OddsProviderBase) en lugar de inspeccionar el nombre de clase.
        Esto mantiene el Orchestrator desacoplado de implementaciones concretas.
        """
        matched = []

        if self.odds_provider.is_test_double:
