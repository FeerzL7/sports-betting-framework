...
class EdgeConfig:
    ...
    """
    market_efficiency: Dict[MarketType, float] = None
    vig_adjustment: bool = True  # Attempt to remove vig from implied probabilities
    
    def __post_init__(self):
        if self.market_efficiency is None:
            # Default efficiency by market (from core.edge.EdgeCalculator)
            self.market_efficiency = {
                MarketType.MONEYLINE: 0.03,         # Very efficient
                MarketType.TOTALS: 0.04,            # Moderately efficient
                MarketType.SPREAD: 0.04,            # Similar to totals
                MarketType.BOTH_TEAMS_SCORE: 0.05,  # Less efficient
                MarketType.PLAYER_PROPS: 0.06       # Least efficient (more value)
            }
